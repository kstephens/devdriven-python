...
def test_json_formatter():
    ...
    assert_message(["MESSAGE"],
                   '"message":"MESSAGE","DATA_KEY":{}')
    with pytest.raises(Exception) as _e_info:
        assert_message(["MESSAGE -> %s %d"],
                       '"message":"MESSAGE","DATA_KEY":{}')
    assert_message(["MESSAGE", "arg1", 2],
                   '"message":"MESSAGE | arg1 | 2","DATA_KEY":{}')
    assert_message(["MESSAGE -> %s %d", "arg1", 2],
                   '"message":"MESSAGE -> arg1 2","DATA_KEY":{}')
    assert_message(["MESSAGE -> %s %d", "arg1", arg_data, 4],
                   '"message":"MESSAGE -> arg1 4",' +
                   '"DATA_KEY":{"rx":"re.compile(\'foo/bar.*\')",' +
                   '"now":"1969-12-31T18:04:06.912000"}')
    assert_message([message_data],
                   '"message":"","DATA_KEY":{"a":1,"b":"str"}')
    assert_message([message_data, 'arg1', 2],
                   '"message":"arg1 | 2","DATA_KEY":{"a":1,"b":"str"}')
    assert_message([message_data, 'arg1', 2, arg_data, 4],
                   '"message":"arg1 | 2 | 4",' +
