...
class FileResponse:
    ...
    def request(self, method, url, headers, body, **kwargs):
        ...
        self.stdin = headers.pop(
            "X-STDIN", kwargs.get("stdin", (FileResponse.default_stdin or sys.stdin))
        )
        self.stdout = headers.pop(
            "X-STDOUT",
            kwargs.get("stdout", (FileResponse.default_stdout or sys.stdout)),
        )
...
def file_etag(path, stat):
    ...
    path_hash.update(
        f"{path}-{stat.st_dev}-{stat.st_ino}-{stat.st_mtime}-{stat.st_size}".encode(
            "utf-8"
        )
    )
