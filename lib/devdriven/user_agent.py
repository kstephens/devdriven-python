...
class UserAgent:
    ...
    def request(self, method, url, headers=None, body=None, **kwargs):
        ...
        for key, val in list(headers.items()):
            if val is None:
                del headers[key]
        return getattr(self, f"_request_scheme_{scheme}")(
            method, url, headers, body, kwargs
        )
    ...
    def _request_scheme_http(self, method, url, headers, body, kwargs):
        ...
        return self.http_pool_manager.request(
            method, url_to_str(url), headers=headers, body=body, **kwargs
        )
...
def with_http_redirects(fun, url, *args, **kwargs):
    ...
    if not completed:
        raise Exception(
            "{url} : status {response and response.status} : Too many redirects : {max_redirects}"
        )
    return response


REDIRECTABLE_STATUS = {301, 302, 305, 307, 308}
