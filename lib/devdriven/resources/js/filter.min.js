...
function filter_load_url_query(){var value=filter_get_url_query();filter_set_url_query(value);if(value!==""){dom_filter_input.value=value;filter_rows_now();}}
...
make_col_maps(table_sel,tr,cols);pin_width(dom_table);var tooltips=table_sel.find(".cx-filter-row .cx-tooltip .cx-tooltip-text");tooltips.forEach(function(item,index,array){item.innerHTML='<pre>'+
help_examples+"\n"+
...
filter_load_url_query();dom_table.addEventListener("afterSort",filter_aria_sort_changed);filter_aria_sort_changed(null);return{table_id:table_id,dom_table:dom_table,dom_cols:dom_cols,dom_ths:dom_ths,dom_rows:dom_rows,col_name_to_idx:col_name_to_idx,col_idx_to_name:col_idx_to_name,filter_rows:filter_rows,filter_rows_now:filter_rows_now,filter_rows_event:filter_rows_event,clear_filter:clear_filter,help_examples:help_examples,help_syntax:help_syntax};}
