...
var name_pat=/^:([^\s:]+)/i;var quote_pat=/^"((\\["\\]|[^"])*)"/;var rx_pat=/^\/((\\\/|[^/])*)\//;var word_pat=/^([^:"\/\s]+)/;function make_parser(){var pc=parser_combininator;function with_type(t,p){return pc.describe('with_type',arguments,pc.when(p,function(str,inp){return{type:t,str:str};}));}
var name_rx=pc.trim(pc.rx(name_pat));var name=pc.pred(name_rx,to_col_idx);var pat_quote=pc.trim(pc.rx(quote_pat));var pat_rx_=pc.trim(pc.rx(rx_pat));var pat_rx=pc.pred(pat_rx_,pc.regexp_maybe);var pat_word=pc.trim(pc.rx(word_pat));var pat=pc.or_(with_type('quote',pat_quote),with_type('rx',pat_rx),with_type('word',pat_word));var col_pat=pc.with_keys(['name','pat'],pc.seq(name,pat));col_pat=pc.when(col_pat,function(pat,inp){return Object.assign({name:pat.name},pat.pat);});var bare_pat=pc.when(pat,function(pat,inp){return Object.assign({name:null},pat);});var p=pc.all(pc.one_or_more(pc.or_(col_pat,bare_pat)));return pc.safe(p);}
...
help_examples=make_html(['  Examples: ','           ','foo        - Any column contains "foo"                       ','foo bar    - Any column contains "foo" followed by "bar"     ','/^foo/     - Any column starts with "foo"  ',':a bar     - Column "a" contains "bar"     ','',]);help_syntax=make_html(['  Syntax:                              ','                                       ','<filter>   = <match> *                 ','<match>    = <all> | <column>          ','<all>      = <pattern>                 ','<column>   = <name> <pattern>          ','<pattern>  = <word> | <rx> | <quote>   ','<name>     = '+name_pat+'           ','<word>     = '+word_pat+'             ','<rx>       = '+rx_pat+'                ','<quote>    = '+quote_pat+'  ','']);function parse_filter_fn_(filter_str){var debug_match=false||debug;var pc=parser_combininator;var result=parser(filter_str);if(!result)return false;var pats_all=result[0];if(pats_all.length==0)return false;var is_bare_pat=pat=>pat.col===null;var bare_pats=pats_all.filter(is_bare_pat);var pats=pats_all.filter(pat=>!is_bare_pat(pat))
...
function each_row(row_fn){if(debug)console.log("psv : filter.js : each_row : {{{ : %d",dom_rows.length);for(var i=0;i<dom_rows.length;i++)
row_fn(dom_rows[i]);if(debug)console.log("psv : filter.js : each_row : }}} : %d",dom_rows.length);}
function reset_rows(){if(debug)console.log("psv : filter.js : reset_rows : {{{ : %d",dom_rows.length);each_row(function(tr){tr.style.display='';});if(debug)console.log("psv : filter.js : reset_rows : }}} : %d",dom_rows.length);dom_matched_row_count.textContent=dom_rows.length;}
function clear_filter(){reset_rows();dom_filter_input.value='';}
function make_row_data(tr){var debug_row_data=false||debug;var tds=Array.from(tr.getElementsByTagName("td"));var row_data=[''];col_idxs.forEach(function(idx){var td=tds[idx];var text=(td.textContent||td.innerText).trim();row_data[idx+1]=text;row_data[0]+=text+' ';});if(debug_row_data)console.log("psv : filter.js : make_row_data = "+JSON.stringify(row_data));return row_data;}
...
return;switch(event.which){case 37:case 39:break;case 38:case 40:break;default:if(debug)console.log("psv : filter.js : event : %s : %s",event,event.which);filter_rows_now();return;}
...
function make_col_maps(table,tr,cols){var debug_cols=false||debug;col_name_to_idx.clear();col_idx_to_name.clear();col_to_data_idx.clear();col_idxs.length=0;var th_idx=-1;$(tr).find('th').each(function(th){var th=this;pin_width(th);th_idx+=1;var name=th.getAttribute("data-filter-name");if(name){col_idxs.push(th_idx);var idx=parseInt(th.getAttribute("data-column-index"));col_to_data_idx.set(name,th_idx+1);if(idx){col_to_data_idx.set(idx,th_idx+1);col_to_data_idx.set(idx+"",th_idx+1);col_name_to_idx.set(name,idx);col_idx_to_name.set(idx,name);col_idx_to_name.set(idx+"",name);}
...
function initalize(table_id_){var debug_init=false||debug;table_id=table_id_;dom_window=window;var table=dom_table=$("#"+table_id);var cols=dom_cols=table.find(".cx-thead .cx-column").toArray();var tr=cols&&cols[0].parentElement;var rows=dom_rows=table.find(".cx-tbody tr").toArray();dom_filter_input=table.find(".cx-thead .cx-filter-input").toArray()[0]
