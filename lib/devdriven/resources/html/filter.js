...
var cx_make_filter =
    function(table_id_) {
      ...
      function each_row(row_fn) {
        if ( debug ) console.log("psv : filter.js : each_row : {{{ : %d", dom_rows.length);
      ...
      function reset_rows() {
        if ( debug ) console.log("psv : filter.js : reset_rows : {{{ : %d", dom_rows.length);
      ...
      function make_row_data(tr) {
        ...
        // if ( debug ) console.log("psv : filter.js : make_row_data = " + JSON.stringify(row_data));
        return row_data;
      }
      ...
      function filter_rows_event(event) {
        ...
        case 40: // - down arrow
          ...
          break;
        default:
          if ( debug ) console.log("psv : filter.js : event : %s : %s", event, event.which);
          filter_rows_now();
          return;
        }
      ...
      function make_col_maps(table, cols) {
        console.log('psv : filter.js : cols = %s', cols);
        ...
        cols.
          ...
          map(function(th) {
            ...
            if ( debug ) console.log("psv : filter.js : %s", JSON.stringify({name: name, idx: idx}));
        ...
        if ( debug ) {
          ...
          console.log("psv : filter.js : col_name_to_idx = %s", col_name_to_idx);
          console.table(col_name_to_idx);
          console.log("psv : filter.js : col_idx_to_name = %s", col_idx_to_name);
          console.table(col_idx_to_name);
        }
      }
      ...
      function initalize(table_id_) {
        ...
        var cols = dom_cols   = table.find(".cx-thead .cx-column").toArray();
