...
class Converter:
  ...
  def as_type(self, data: Any, typ: Union[str, Type, None], *args):
    ...
    if isinstance(typ, TYPE):
      typ = typ.__name__
    if fun := getattr(self, f'as_{typ}', None):
  ...
  def as_iterable(self, data: Any, elem_type: Optional[str] = None) -> Iterable:
    ...
    if isinstance(data, dict):
      ...
      return [self.as_type(elem, elem_type)
  ...
  def as_dict(self, data: Any, key_type: Optional[str] = None, val_type: Optional[str] = None) -> dict:
    if isinstance(data, dict):
      if key_type is None and val_type is None:
        return data
      return {self.as_type(k, key_type): self.as_type(v, val_type) for k, v in data.items()}
    if isinstance(data, list):
