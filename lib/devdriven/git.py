...
GIT_REPO_SSH_RX = re.compile(
    r"^(?P<user>[^@]+)@(?P<host>[^:]+):(?P<org>[^/]+)/(?P<repo>.+?)\.git$"
)
...
class GitCommit:
    def __init__(self, directory: str, line: str):
        ...
        self.ref, self.timestamp, self.committer_email, self.subject = line.split(
            "\t", 4
        )
...
class GitLog:
    ...
    def parse_lines(self, lines: List[str]) -> List[GitCommit]:
        ...
        return sorted(
            [GitCommit(self.directory, line) for line in lines],
