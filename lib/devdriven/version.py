...
class Version:
  ...
  def cmp_right(self, ver2: Any, _op: str = '') -> int:
    return cmp_list_right(self.elems, ver2.elems)


CoerceableVersion = Union[Version, str]

def typecheck_op(self: Version, other: Any, name: str) -> Any:
...
def coerce_to_version(ver: Any) -> Version:
...
def cmp_list(a: Indexable, b: Indexable) -> int:
...
def cmp_list_right(a: Indexable, b: Indexable) -> int:
...
class VersionConstraintRelation:
  ...
  def __init__(self, other: Union[Self, str]):
    if isinstance(other, VersionConstraintRelation):
      ...
      self.oper, self.version = other.oper, other.version
...
def parse_version_constraint(con: str) -> Tuple[str, Version]:
  if m := re.match(VERSION_CONSTRAINT_ELEMENT_RX, con):
